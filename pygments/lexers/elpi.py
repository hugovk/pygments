...
class ElpiLexer(RegexLexer):
    ...
    digit_re = r"[0-9]"
    schar2_re = r"([+*^?/<>`'@#~=&!])"
    schar_re = fr"({schar2_re}|-|\$|_)"
    idchar_re = fr"({lcase_re}|{ucase_re}|{digit_re}|{schar_re})"
    idcharstarns_re = fr"({idchar_re}*(\.({lcase_re}|{ucase_re}){idchar_re}*)*)"
    symbchar_re = fr"({lcase_re}|{ucase_re}|{digit_re}|{schar_re}|:)"
    constant_re = fr"({ucase_re}{idchar_re}*|{lcase_re}{idcharstarns_re}|{schar2_re}{symbchar_re}*|_{idchar_re}+)"
    symbol_re = r"(,|<=>|->|:-|;|\?-|->|&|=>|\bas\b|\buvar\b|<|=<|=|==|>=|>|\bi<|\bi=<|\bi>=|\bi>|\bis\b|\br<|\br=<|\br>=|\br>|\bs<|\bs=<|\bs>=|\bs>|@|::|\[\]|`->|`:|`:=|\^|-|\+|\bi-|\bi\+|r-|r\+|/|\*|\bdiv\b|\bi\*|\bmod\b|\br\*|~|\bi~|\br~)"
    escape_re = fr"\(({constant_re}|{symbol_re})\)"
    const_sym_re = fr"({constant_re}|{symbol_re}|{escape_re})"
    ...
    tokens = {
        ...
        'elpi': [
            ...
            (r"(:index)(\s*\()", bygroups(Keyword.Mode, Text.Whitespace),
             'elpi-indexing-expr'),
            (fr"\b(external pred|pred)(\s+)({const_sym_re})",
             bygroups(Keyword.Declaration, Text.Whitespace, Name.Function),
             'elpi-pred-item'),
            (fr"\b(external type|type)(\s+)(({const_sym_re}(,\s*)?)+)",
             bygroups(Keyword.Declaration, Text.Whitespace, Name.Function),
             'elpi-type'),
            (fr"\b(kind)(\s+)(({const_sym_re}|,)+)",
             bygroups(Keyword.Declaration, Text.Whitespace, Name.Function),
             'elpi-type'),
            (fr"\b(typeabbrev)(\s+)({const_sym_re})",
             bygroups(Keyword.Declaration, Text.Whitespace, Name.Function),
             'elpi-type'),
            (r"\b(accumulate)(\s+)(\")",
             bygroups(Keyword.Declaration, Text.Whitespace, String.Double),
             'elpi-string'),
            (fr"\b(accumulate|namespace|local)(\s+)({constant_re})",
             bygroups(Keyword.Declaration, Text.Whitespace, Text)),
            (fr"\b(shorten)(\s+)({constant_re}\.)",
             bygroups(Keyword.Declaration, Text.Whitespace, Text)),
            (r"\b(pi|sigma)(\s+)([a-zA-Z][A-Za-z0-9_ ]*)(\\)",
             bygroups(Keyword.Declaration, Text.Whitespace, Name.Variable, Text)),
            (fr"\b(constraint)(\s+)(({const_sym_re}(\s+)?)+)",
             bygroups(Keyword.Declaration, Text.Whitespace, Name.Function),
             'elpi-chr-rule-start'),

            (fr"(?=[A-Z_]){constant_re}", Name.Variable),
            (fr"(?=[a-z_]){constant_re}\\", Name.Variable),
            (r"_", Name.Variable),
            (fr"({symbol_re}|!|=>|;)", Keyword.Declaration),
            (constant_re, Text),
        ...
        'elpi-quote': [
            (r'\{\{', Punctuation, '#push'),
            (r'\}\}', Punctuation, '#pop'),
            (fr"(lp:)((?=[A-Z_]){constant_re})", bygroups(Keyword, Name.Variable)),
            (r"[^l\}]+", Text),
            (r"l|\}", Text),
        ],
        'elpi-spill': [
