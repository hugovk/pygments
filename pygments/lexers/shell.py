...
class BatchLexer(RegexLexer):
    ...
    _ws = r'\t\v\f\r ,;=\xa0'
    _nlws = r'\s\x1a\xa0,;='
    _space = r'(?:(?:(?:\^[{}])?[{}])+)'.format(_nl, _ws)
    _keyword_terminator = (r'(?=(?:\^[%s]?)?[%s+./:[\\\]]|[%s%s(])' %
                           (_nl, _ws, _nl, _punct))
    _token_terminator = r'(?=\^?[{}]|[{}{}])'.format(_ws, _punct, _nl)
    _start_label = r'((?:(?<=^[^:])|^[^:]?)[%s]*)(:)' % _ws
    _label = r'(?:(?:[^{}{}+:^]|\^[{}]?[\w\W])*)'.format(_nlws, _punct, _nl)
    _label_compound = r'(?:(?:[^{}{}+:^)]|\^[{}]?[^)])*)'.format(_nlws, _punct, _nl)
    _number = r'(?:-?(?:0[0-7]+|0x[\da-f]+|\d+)%s)' % _token_terminator
    _opword = r'(?:equ|geq|gtr|leq|lss|neq)'
    _string = r'(?:"[^{}"]*(?:"|(?=[{}])))'.format(_nl, _nl)
    _variable = (r'(?:(?:%%(?:\*|(?:~[a-z]*(?:\$[^:]+:)?)?\d|'
                 r'[^%%:%s]+(?::(?:~(?:-?\d+)?(?:,(?:-?\d+)?)?|(?:[^%%%s^]|'
                 r'\^[^%%%s])[^=%s]*=(?:[^%%%s^]|\^[^%%%s])*)?)?%%))|'
                 r'(?:\^?![^!:%s]+(?::(?:~(?:-?\d+)?(?:,(?:-?\d+)?)?|(?:'
    ...
    _core_token = r'(?:(?:(?:\^[{}]?)?[^"{}{}])+)'.format(_nl, _nlws, _punct)
    _core_token_compound = r'(?:(?:(?:\^[{}]?)?[^"{}{})])+)'.format(_nl, _nlws, _punct)
    _token = r'(?:[{}]+|{})'.format(_punct, _core_token)
    _token_compound = r'(?:[{}]+|{})'.format(_punct, _core_token_compound)
    _stoken = (r'(?:[%s]+|(?:%s|%s|%s)+)' %
    ...
    def _make_begin_state(compound, _core_token=_core_token,
        ...
        rest = '(?:{}|{}|[^"%{}{}{}])*'.format(_string, _variable, _nl, _punct,
                                            ')' if compound else '')
        rest_of_line = r'(?:(?:[^{}^]|\^[{}]?[\w\W])*)'.format(_nl, _nl)
        rest_of_line_compound = r'(?:(?:[^{}^)]|\^[{}]?[^)])*)'.format(_nl, _nl)
        set_space = r'((?:(?:\^[%s]?)?[^\S\n])*)' % _nl
        suffix = ''
        if compound:
        ...
        return [
            ((r'\)', Punctuation, '#pop') if compound else
             (r'\)((?=\()|{}){}'.format(_token_terminator, rest_of_line),
              Comment.Single)),
            ...
            (r'(goto{})({}?)(:?)'.format(_keyword_terminator, _space),
             bygroups(Keyword, using(this, state='text'), Punctuation),
             'label%s' % suffix),
            ...
            (r'(set{}){}(/a)'.format(_keyword_terminator, set_space),
             bygroups(Keyword, using(this, state='text'), Keyword),
             'arithmetic%s' % suffix),
    ...
    tokens = {
        ...
        'for2': [
            (r'\)', Punctuation),
            (r'({})(do{})'.format(_space, _token_terminator),
             bygroups(using(this, state='text'), Keyword), '#pop'),
            (r'[%s]+' % _nl, Text),
            include('follow')
        ],
        'for/f': [
            (r'(")((?:{}|[^"])*?")([{}]*)(\))'.format(_variable, _nlws),
             bygroups(String.Double, using(this, state='string'), Text,
                      Punctuation)),
            (r'"', String.Double, ('#pop', 'for2', 'string')),
            (r"('(?:%%|{}|[\w\W])*?')([{}]*)(\))".format(_variable, _nlws),
